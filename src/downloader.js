...
export async function fetchVideoMetadata(postUrl) {
  ...
  const metadata = {
    ...
    duration_seconds: Number.isFinite(parsed.duration) ? parsed.duration : null,
  };

  // yt-dlp does not expose view counts for Instagram clips. Fall back to
  // Instagram's media-info endpoint, which still reports them as play_count.
  if (metadata.view_count == null) {
    const views = await fetchInstagramViewCount(postUrl);
    if (Number.isFinite(views)) metadata.view_count = views;
  }
